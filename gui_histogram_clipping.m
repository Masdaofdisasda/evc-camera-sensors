...
if handles.maxVal == 0
    ...
    limits = [-handles.minBorder * handles.maxVal, handles.maxVal * handles.maxBorder];
end
set(handles.ax_histoClip, 'XLim', limits);
...
if isappdata(handles.ax_pictureClip, 'hist_max') %wait until everything is initialized
    ...
    if (m > 0 && (click >= 0) && (coords(1, 1) >= xlim(1)) && (coords(1, 1) <= xlim(2)) && (coords(1, 2) >= -0.1) && (coords(1, 2) <= hm))
        ...
        if (getappdata(handles.bt_okClip, 'clickNr') == 1)
            ...
            if ((abs(x-x1) > 0.001) && (x1 < x))
                ...
                if m == 0
                    ...
                    limits = [-handles.minBorder * m, m * handles.maxBorder];
                end
                set(handles.ax_histoClip, 'XLim', limits);
